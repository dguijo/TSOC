...
__all__=["OrdinalShapeletTransformFisherOrd","ContractedOrdinalShapeletTransformFisherOrd","RandomEnumerationOrdinalShapeletTransformFisherOrd","Shapelet","ShapeletPQ"]
...
class OrdinalShapeletTransformFisherOrd(BaseTransformer):
    ...
    def fit(self, X, y=None):
        ...
        if type(self) is RandomEnumerationOrdinalShapeletTransformFisherOrd:
        ...
        if type(self) is RandomEnumerationOrdinalShapeletTransformFisherOrd or type(self) is ContractedOrdinalShapeletTransformFisherOrd:
        ...
        while case_idx < len(cases_to_visit):
            ...
            if self.verbose:
                if type(self) == RandomEnumerationOrdinalShapeletTransformFisherOrd:
            ...
            for candidate_idx in range(num_candidates_per_case):
                ...
                candidate = OrdinalShapeletTransformFisherOrd.zscore(X[series_id][:,cand_start_pos: cand_start_pos + cand_len])
                ...
                for comparison_series_idx in range(len(cases_to_visit)):
                    ...
                    for num_cals in range(max(1,int(np.ceil((X_lens[i]-cand_len)/2)))): # max used to force iteration where series len == candidate len
                        ...
                        comparison = OrdinalShapeletTransformFisherOrd.zscore(X[i][:,start_left: start_left+ cand_len])
                        ...
                        if start_right == X_lens[i]-cand_len+1:
                            start_right = 0
                        comparison = OrdinalShapeletTransformFisherOrd.zscore(X[i][:,start_right: start_right + cand_len])
                ...
                if candidate_rejected is False:
                    final_ig = OrdinalShapeletTransformFisherOrd.calc_fisher_ordinal(orderline)
        ...
        for class_val in distinct_class_vals:
            ...
            if self.remove_self_similar and len(by_class_descending_ig) > 0:
                by_class_descending_ig = OrdinalShapeletTransformFisherOrd.remove_self_similar_shapelets(by_class_descending_ig)
            else:
        ...
        for i in range(0, len(X)):
            ...
            for s in range(0, len(self.shapelets)):
                ...
                for start_pos in range(0, len(this_series[0]) - this_shapelet_length + 1):
                    comparison = OrdinalShapeletTransformFisherOrd.zscore(this_series[:, start_pos:start_pos + this_shapelet_length])
    ...
    def calc_binary_ig(orderline, total_num_this_class, total_num_other_class):
        ...
        initial_ent = OrdinalShapeletTransformFisherOrd.binary_entropy(total_num_this_class, total_num_other_class)
        bsf_ig = 0
        ...
        for split in range(0, len(orderline) - 1):
            ...
            ent_left = OrdinalShapeletTransformFisherOrd.binary_entropy(count_this_class,count_other_class)

            right_prop = 1-left_prop # because right side must optimistically contain everything else
            ent_right = OrdinalShapeletTransformFisherOrd.binary_entropy(total_num_this_class-count_this_class,total_num_other_class-count_other_class)
    ...
    def calc_early_binary_ig(orderline, num_this_class_in_orderline, num_other_class_in_orderline, num_to_add_this_class, num_to_add_other_class):
        ...
        initial_ent = OrdinalShapeletTransformFisherOrd.binary_entropy(num_this_class_in_orderline+num_to_add_this_class, num_other_class_in_orderline+num_to_add_other_class)
        bsf_ig = 0
        ...
        for split in range(0, len(orderline) - 1):
            ...
            ent_left = OrdinalShapeletTransformFisherOrd.binary_entropy(count_this_class+num_to_add_this_class,count_other_class)

            right_prop = 1-left_prop # because right side must optimistically contain everything else
            ent_right = OrdinalShapeletTransformFisherOrd.binary_entropy(num_this_class_in_orderline-count_this_class,num_other_class_in_orderline-count_other_class+num_to_add_other_class)
            ...
            ent_left = OrdinalShapeletTransformFisherOrd.binary_entropy(count_this_class,count_other_class+num_to_add_other_class)

            right_prop = 1-left_prop # because right side must optimistically contain everything else
            ent_right = OrdinalShapeletTransformFisherOrd.binary_entropy(num_this_class_in_orderline-count_this_class+num_to_add_this_class,num_other_class_in_orderline-count_other_class)
            ig = initial_ent - left_prop * ent_left - right_prop * ent_right
            bsf_ig = max(ig, bsf_ig)
...
class ContractedOrdinalShapeletTransformFisherOrd(OrdinalShapeletTransformFisherOrd):
...
class RandomEnumerationOrdinalShapeletTransformFisherOrd(OrdinalShapeletTransformFisherOrd):
    pass
    # to follow

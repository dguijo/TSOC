...
class OrdinalShapeletTransformRegLin(BaseTransformer):
        ...
        while case_idx < len(cases_to_visit):
            ...
            for candidate_idx in range(num_candidates_per_case):
                ...
                if candidate_rejected is False:
                    final_ig = OrdinalShapeletTransformRegLin.calc_correlation_regLin(orderline, y[series_id])
    ...
    @staticmethod
    def calc_correlation_regLin(orderline, shp_class):

        orderline = np.array(orderline)

        _, _, r_value, _, _ = linregress(orderline[:, 0], np.abs(orderline[:, 1] - shp_class))
        return r_value ** 2

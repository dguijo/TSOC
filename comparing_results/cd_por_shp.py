...
classifiers = ["svc1v1", "svc1va", "svorim", "pom"]
    ...
    for x, i in enumerate(shp_type):
        for j in classifiers:
            for k in datasets:
                ...
                if maximizar:
                    metric = -metric
                matrix.append([str(i), k + "_" + j, metric])
    name_file = "results_shp_" + metricName + ".csv"
    pd.DataFrame(matrix).to_csv(name_file, header=None, index=None)

    df_perf = pd.read_csv(name_file, index_col=False)
    draw_cd_diagram(df_perf=df_perf, name_figure="shp_" + metricName + ".png", metric=metricName)